...
#region Members
	...
	[SerializeField]
	List<SerializableLocalizationObjectPair> thisLanguageValues = new List<SerializableLocalizationObjectPair>();	
	[SerializeField]
	string thisLanguage;
	[SerializeField]
	CultureInfo thisCultureInfo;
	...
	bool canLanguageBeTranslated;
	///<summary> Languages available to translate from</summary>
	List<CultureInfo> availableTranslateFromLanguages = new List<CultureInfo>();
	...
	int translateFromLanguageValue;
	int oldTranslateFromLanguageValue;
	...
	SmartLocalizationWindow smartLocWindow;
#endregion
#region Initialization
	...
	void InitializeLanguage (Dictionary< string, LocalizedObject> rootValues, Dictionary <string, LocalizedObject> thisLanguageValues) {
		InitializeLanguage (null, rootValues, thisLanguageValues);
	}
	...
	void InitializeLanguage(CultureInfo info, Dictionary<string, LocalizedObject> rootValues, Dictionary<string, LocalizedObject> thisLanguageValues)
	{
		...
		thisLanguage = (thisCultureInfo.EnglishName + " - " + thisCultureInfo.Name);
...
#region GUI Functions
	...
	/// Root value.
	/// </param>
	void OnTextFieldGUI(KeyValuePair<string,LocalizedObject> rootValue, int iterationCount)
	{
		EditorGUILayout.BeginHorizontal();
	...
	/// Root value.
	/// </param>
	void OnGameObjectGUI(KeyValuePair<string,LocalizedObject> rootValue, int iterationCount)
	{
		EditorGUILayout.BeginHorizontal();
		GUILayout.Label(rootValue.Key, GUILayout.Width(120));
		GUILayout.Label(rootValue.Value.TextValue,GUILayout.Width(120));
	...
	/// Root value.
	/// </param>
	void OnAudioGUI(KeyValuePair<string,LocalizedObject> rootValue, int iterationCount)
	{
		EditorGUILayout.BeginHorizontal();
		GUILayout.Label(rootValue.Key, GUILayout.Width(120));
		GUILayout.Label(rootValue.Value.TextValue,GUILayout.Width(120));
	...
	/// Root value.
	/// </param>
	void OnTextureGUI(KeyValuePair<string,LocalizedObject> rootValue, int iterationCount)
	{
		EditorGUILayout.BeginHorizontal();
		GUILayout.Label(rootValue.Key, GUILayout.Width(120));
		GUILayout.Label(rootValue.Value.TextValue,GUILayout.Width(120));
...
#region Event Callbacks
    ...
    {
		TranslateLanguageWindow thisWindow = EditorWindow.GetWindow<TranslateLanguageWindow> ("Translate Language", true, typeof (SmartLocalizationWindow));
		thisWindow.smartLocWindow = smartLocWindow;
		thisWindow.Initialize (info, false);
		return thisWindow;
    }
}
